...
### Note
The tool will work against standard exports like ```SAM``` or ```SECURITY```, but is unable to obtain the system BOOTKEY from a ```SYSTEM``` export. The limitations for ```SYSTEM``` are inherent to the implementation of the ```.reg``` export process. The required class metadata is not present in the ```.reg``` file at all and thus, cannot be used to recover the bootkey.
